...
class OrganizationMemberWithProjectsSerializer(OrganizationMemberSerializer):
    ...
    def get_attrs(self, item_list, user):
        ...
        # `.prefetch_related('teams', 'teams__projectteam_set')` on your
        # queryset before using this serializer
        for org_member in item_list:
            ...
            for team in org_member.teams.all():
                ...
                for project_team in team.projectteam_set.all():
                    if project_team.project_id in self.project_ids:
                        projects.add(project_team.project_id)

            projects = list(projects)
            projects.sort()
            attrs[org_member]['projects'] = map(six.text_type, projects)
